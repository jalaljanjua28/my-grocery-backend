...
def food_waste_reduction_using_json():
    try:
        data = core.get_data_from_json('ChatGPT/HomePage', 'Food_Waste_Reduction_Suggestions')
        if isinstance(data, dict) and 'error' in data:
            return jsonify({'error': data['error']}), 500
...
def health_incompatibilities_using_json():
    try:
        data = core.get_data_from_json('ChatGPT/Health', 'health_incompatibility_information_all')
        if isinstance(data, dict) and 'error' in data:
            return jsonify({'error': data['error']}), 500
