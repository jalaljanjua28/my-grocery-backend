        ...
        for category, items in master_data.items():
            for item in items:
                if item["Name"].lower() == item_name:
                    ...
                    core.save_data_to_cloud_storage("ItemsList", slave_list_name, response)
...
def move_to_food():
    ...
    try:
        ...
        core.save_data_to_cloud_storage("ItemsList", "master_nonexpired", nonexpired_data)
...
def update_item_name():
    ...
    try:
        ...
        core.save_data_to_cloud_storage("ItemsList", "master_nonexpired", nonexpired_content)
...
def delete_all_items(file_type):
    ...
    try:
        ...
        data["Not_Food"] = [item for item in data["Not_Food"] if item.get("Name") == "TestFNE"]
        ...
        for category, items in master_data.items():
            for item in items:
                if item.get("Name", "").lower() == item_name:
                    try:
                        expiry_date = datetime.strptime(item.get("Expiry_Date", "1/1/2026"), "%d/%m/%Y")
                        ...
                        if "Days_Until_Expiry" in item:
                            item['Days_Until_Expiry'] = item['Days_Until_Expiry'] + days_to_extend
...
def check_frequency():
    ...
    try:
        ...
        if choice == 'biweekly':
            ...
            return jsonify({"error": "Invalid condition. Use 'biweekly', 'monthly', or 'today'."}), 400
        ...
        core.save_data_to_cloud_storage("ItemsList", "item_frequency_sorted", sorted_item_frequency)
